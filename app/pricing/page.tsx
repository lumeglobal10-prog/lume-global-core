...
// Corrección de ruta: subimos dos niveles para llegar a components
...
export default function PricingPage() {
  ...
  const proceedToCheckout = () => {
    if (!isLoggedIn) {
      ...
      // Aquí conectaremos con la pasarela de pagos
    }
  };
  ...
  return (
    <main className="min-h-screen bg-white text-black font-sans flex flex-col p-6 md:p-12">
      ...
      {/* Módulo de Interrupción Inteligente */}
      {showTaxPopup && (
        <SmartPopup 
          trigger="tax_notice" 
      ...
      <nav className="flex justify-between items-center w-full mb-12">
        ...
        {isLoggedIn && (
          <button onClick={() => setIsLoggedIn(false)} className="text-[10px] font-bold text-red-500 uppercase">
            Cerrar Sesión
          </button>
        )}
      </nav>
